...
import com.example.a214188_radhiahjamalludin_drnazatul_project2.data.local.CourseDao
...
class CourseRepository(private val courseDao: CourseDao) {
    ...
    // Favourite Tips
    val allFavouriteTips: Flow<List<FavouriteTipEntity>> = courseDao.getAllFavouriteTips()

    suspend fun insertFavouriteTip(tip: FavouriteTipEntity) {
        courseDao.insertFavouriteTip(tip)
    }

    suspend fun deleteFavouriteTip(tip: FavouriteTipEntity) {
        courseDao.deleteFavouriteTip(tip)
    }

    suspend fun deleteAllFavouriteTips() {
        courseDao.deleteAllFavouriteTips()
    }
}
